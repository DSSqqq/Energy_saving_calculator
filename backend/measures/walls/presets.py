"""
Пресеты для разных типов зданий.

Значения соответствуют примеру из исходного аудита:
- Общественные здания: отопительный период 204 суток, средняя t_нар = -7,1 °C.
- Производственные:    158 суток, -10,0 °C.

UI заполняет ими поля по умолчанию, но пользователь может переопределить вручную
для конкретного здания (другой регион, особый режим работы и т.п.).
"""
